...
static bool lpc_mii_write(uint8_t reg, uint16_t data)
{
	bool sts = false;
	int32_t mst = 250;

	/* Write value for register */
	Chip_ENET_StartMIIWrite(LPC_EMAC, reg, data);

	/* Wait for unbusy status */
	while (mst > 0) {
		if (Chip_ENET_IsMIIBusy(LPC_EMAC)) {
			mst--;
			msDelay(1);
		}
		else {
...
/* Read from the PHY. Will block for delays based on the pDelayMs function. Returns
   true on success, or false on failure */
static bool lpc_mii_read(uint8_t reg, uint16_t *data)
{
	bool sts = false;
	int32_t mst = 250;

	/* Start register read */
	Chip_ENET_StartMIIRead(LPC_EMAC, reg);

	/* Wait for unbusy status */
	while (mst > 0) {
		if (!Chip_ENET_IsMIIBusy(LPC_EMAC)) {
			mst = 0;
			*data = Chip_ENET_ReadMIIData(LPC_EMAC);
			sts = true;
		}
		else {
			mst--;
...
uint32_t lpc_phy_init(bool rmii, uint32_t pDelayMsFunc)
{
	uint16_t tmp;
	int32_t i;
	...
	if (lpc_mii_write(LAN8_BCR_REG, LAN8_RESET) == false) {
		return false;
	}
	i = 400;
	while (i > 0) {
		msDelay(1);
		if (lpc_mii_read(LAN8_BCR_REG, &tmp) == false) {
			return false;
		}
...
uint32_t lpcPHYStsPoll(void)
{
	...
	case 0:
		...
		Chip_ENET_StartMIIRead(LPC_EMAC, LAN8_BSR_REG);
		physts &= ~PHY_LINK_CHANGED;
	...
	case 1:
		...
		if (!Chip_ENET_IsMIIBusy(LPC_EMAC)) {
			...
			sts = Chip_ENET_ReadMIIData(LPC_EMAC);
			Chip_ENET_StartMIIRead(LPC_EMAC, LAN8_PHYSPLCTL_REG);
			phyustate = 2;
		}
		break;
	...
	case 2:
		...
		if (!Chip_ENET_IsMIIBusy(LPC_EMAC)) {
			...
			smsc_update_phy_sts(sts, Chip_ENET_ReadMIIData(LPC_EMAC));
			phyustate = 0;
		}
		break;
	}
