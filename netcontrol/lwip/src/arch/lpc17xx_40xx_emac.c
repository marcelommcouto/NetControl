...
static err_t lpc_rx_setup(lpc_enetdata_t *lpc_enetif)
{
	...
	Chip_ENET_InitRxDescriptors(LPC_EMAC, lpc_enetif->prxd, lpc_enetif->prxs, LPC_NUM_BUFF_RXDESCS);
...
static struct pbuf *lpc_low_level_input(struct netif *netif) {
	...
	if (Chip_ENET_GetIntStatus(LPC_EMAC) & ENET_INT_RXOVERRUN) {
		LINK_STATS_INC(link.err);
		LINK_STATS_INC(link.drop);

		/* Temporarily disable RX */
		Chip_ENET_RXDisable(LPC_EMAC);

		/* Reset the RX side */
		Chip_ENET_ResetRXLogic(LPC_EMAC);
		Chip_ENET_ClearIntStatus(LPC_EMAC, ENET_INT_RXOVERRUN);
	...
	length = 0;
	idx = Chip_ENET_GetRXConsumeIndex(LPC_EMAC);
	if (!Chip_ENET_IsRxEmpty(LPC_EMAC)) {
		/* Handle errors */
		...
		Chip_ENET_IncRXConsumeIndex(LPC_EMAC);
	}
...
static err_t lpc_tx_setup(lpc_enetdata_t *lpc_enetif)
{
	...
	Chip_ENET_InitTxDescriptors(LPC_EMAC, lpc_enetif->ptxd, lpc_enetif->ptxs, LPC_NUM_BUFF_TXDESCS);
...
static err_t lpc_low_level_output(struct netif *netif, struct pbuf *p)
{
	...
	idx = Chip_ENET_GetTXProduceIndex(LPC_EMAC);
	...
	while (dn > 0) {
		...
		idx = Chip_ENET_IncTXProduceIndex(LPC_EMAC);
	}
...
static void vPacketReceiveTask(void *pvParameters)
{
	...
	while (1) {
		...
		while (!Chip_ENET_IsRxEmpty(LPC_EMAC)) {
...
static void vTransmitCleanupTask(void *pvParameters)
{
	...
	while (1) {
		...
		if (Chip_ENET_GetIntStatus(LPC_EMAC) & ENET_INT_TXUNDERRUN) {
			LINK_STATS_INC(link.err);
			LINK_STATS_INC(link.drop);
...
#endif
			/* Reset the TX side */
			Chip_ENET_ResetTXLogic(LPC_EMAC);
			Chip_ENET_ClearIntStatus(LPC_EMAC, ENET_INT_TXUNDERRUN);
...
static err_t low_level_init(struct netif *netif)
{
...
#if defined(USE_RMII)
	Chip_ENET_Init(LPC_EMAC, true);

#else
	Chip_ENET_Init(LPC_EMAC, false);
#endif

	/* Initialize the PHY */
	Chip_ENET_SetupMII(LPC_EMAC, Chip_ENET_FindMIIDiv(LPC_EMAC, 2500000), LPC_PHYDEF_PHYADDR);
#if defined(USE_RMII)
	if (lpc_phy_init(true, msDelay) == false) {
		return false;
	}
#else
	if (lpc_phy_init(false, msDelay) == 0) {
		return 0;
	}
#endif

	/* Save station address */
	Chip_ENET_SetADDR(LPC_EMAC, netif->hwaddr);
	...
	Chip_ENET_EnableRXFilter(LPC_EMAC, ENET_RXFILTERCTRL_APE | ENET_RXFILTERCTRL_ABE);
#else
	Chip_ENET_EnableRXFilter(ENET_RXFILTERCTRL_APE);
#endif

	/* Clear and enable rx/tx interrupts */
	Chip_ENET_EnableInt(LPC_EMAC, RXINTGROUP | TXINTGROUP);

	/* Enable RX and TX */
	Chip_ENET_TXEnable(LPC_EMAC);
	Chip_ENET_RXEnable(LPC_EMAC);
...
void lpc_tx_reclaim(struct netif *netif)
{
	lpc_tx_reclaim_st((lpc_enetdata_t *) netif->state,
					  Chip_ENET_GetTXConsumeIndex(LPC_EMAC));
}
...
s32_t lpc_tx_ready(struct netif *netif)
{
	...
	cidx = Chip_ENET_GetTXConsumeIndex(LPC_EMAC);
	pidx = Chip_ENET_GetTXProduceIndex(LPC_EMAC);

	return Chip_ENET_GetFreeDescNum(LPC_EMAC, pidx, cidx, LPC_NUM_BUFF_TXDESCS);
}
...
#endif
{
	...
	ints = Chip_ENET_GetIntStatus(LPC_EMAC);
	...
	Chip_ENET_ClearIntStatus(LPC_EMAC, ints);
...
err_t lpc_enetif_init(struct netif *netif)
{
	...
	memcpy(netif->hwaddr, boardmac, 6);
	...
	sys_thread_new("txclean_thread", vTransmitCleanupTask, netif->state, DEFAULT_THREAD_STACKSIZE, tskTXCLEAN_PRIORITY);
#endif
